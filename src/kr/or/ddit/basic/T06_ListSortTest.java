...
public class T06_ListSortTest {
	 ...
	 *  	    정렬 기준을 별도로 구현하고 싶을때는 Comparator를 구현해서 사용하면 된다.
	 *  	
	...
	public static void main(String[] args) {
		...
		// 정렬은 기본적으로 '오름차순 정렬'을 수행한다.( a -> z )

		// 정렬방식을 변경하려면 정렬방식을 결정하는 객체를 만들어서
		// Collections.sort()메서드에 인수로 넘겨주면 된다.

		// Collections는 클래스임. util 기능을 구현해놨다.
		// sort가 가능한 이유는 String이 comparable을 내부적으로 구현해놨으므로.
		...
		// Desc에서는 정렬(comparable)을 구현 안해놨으므로 이렇게 사용하면 정렬가능
 ...
 * 이 Comparator인터페이스의 compare()라는 메서드를 재정의(오버라이딩) 하여 구현하면 된다. 
 * @author Hyungwook
 *
 */
class Desc implements Comparator<String> {
	 ...
	 *    
	 *   - String객체에서는 정렬을 위해서 compareTo()메서드가 구현되어 있는데 이 메서드의
	 *     반환값은 오름차순에 맞게 반환되도록 구현되어 있다.
	 *     (Wrapper클래스와 Date, File클래스에서도 구현되어 있다.)
	 */
	
	@Override
	public int compare(String str1, String str2) {
		return str1.compareTo(str2) * -1; // String객체에서 제공하는compareTo를 사용 -> 호출해서 그냥 갖다쓰면 됨
		// 원래는 오름차순인데 -1을 곱해주면 내림차순으로 바뀜.
	}
